...
def detect_face(img):
    ...
    face_hull = cv2.convexHull(
        np.array([[landmarks.part(n).x, landmarks.part(n).y] for n in range(0, 81)]))

    lebrow_hull = cv2.convexHull(
        np.array([[landmarks.part(n).x, landmarks.part(n).y] for n in range(17, 22)]))
    rebrow_hull = cv2.convexHull(
        np.array([[landmarks.part(n).x, landmarks.part(n).y] for n in range(22, 27)]))

    leye_hull = cv2.convexHull(
        np.array([[landmarks.part(n).x, landmarks.part(n).y] for n in range(36, 42)]))
    reye_hull = cv2.convexHull(
        np.array([[landmarks.part(n).x, landmarks.part(n).y] for n in range(42, 48)]))
    nose_hull = cv2.convexHull(
        np.array([[landmarks.part(n).x, landmarks.part(n).y] for n in range(27, 36)]))
    lip_hull = cv2.convexHull(
        np.array([[landmarks.part(n).x, landmarks.part(n).y] for n in range(48, 68)]))
    ...
    lebrow_mask = cv2.fillConvexPoly(
        lebrow_mask, lebrow_hull, 0).astype(np.uint8)
    rebrow_mask = cv2.fillConvexPoly(
        rebrow_mask, rebrow_hull, 0).astype(np.uint8)
...
def detect_acne(face, orig_img):
    ...
    contours, hierarchy = cv2.findContours(
        th3.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
...
def extractor():
    ...
    output_img.save('../src/assets/output.png')
    return jsonify({"AcneScore": acne_area*100})
